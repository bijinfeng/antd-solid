...
export type SemanticStylesType<
  Props extends AnyObject,
  SemanticStyles extends Record<PropertyKey, JSX.CSSProperties>,
  NestedStructure extends EmptyObject = EmptyObject,
...
export const mergeStyles = <StylesType extends AnyObject>(
  ...
  return styles
    .filter(Boolean)
    .reduce<Record<PropertyKey, JSX.CSSProperties>>((acc, cur = {}) => {
      Object.keys(cur).forEach((key) => {
...
>(
  ...
  const resolvedClassNamesList = createMemo(() => classNamesList.map((classNames) =>
    classNames ? resolveStyleOrClass(classNames, info) : undefined,
  ));

  const resolvedStylesList = createMemo(() => stylesList.map((styles) =>
    styles ? resolveStyleOrClass(styles, info) : undefined,
  ));

  const mergedClassNames = createMemo(() => {
    const classNames = useSemanticClassNames(
      schema,
      ...resolvedClassNamesList(),
    ) as ObjectOnly<ClassNamesType>;
    return schema ? fillObjectBySchema<ObjectOnly<ClassNamesType>>(classNames, schema) : classNames;
  });

  const mergedStyles = createMemo(() => {
    const styles = useSemanticStyles(...resolvedStylesList()) as ObjectOnly<StylesType>;
    return schema ? fillObjectBySchema<ObjectOnly<StylesType>>(styles, schema) : styles;
  })
