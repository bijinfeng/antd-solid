...
// =========================== Export ===========================
const fillObjectBySchema = <T extends AnyObject>(
	obj: T,
	schema: SemanticSchema,
): T => {
...
>(
	...
	const mergedClassNames = () => {
		...
		return schema
			? fillObjectBySchema<ObjectOnly<ClassNamesType>>(classNames, schema)
			: classNames;
	};

	const mergedStyles = () => {
		const styles = useSemanticStyles(
			...resolvedStylesList,
		) as ObjectOnly<StylesType>;
		return schema
			? fillObjectBySchema<ObjectOnly<StylesType>>(styles, schema)
			: styles;
	};
