// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ForwardOutlinedSvg from '@ant-design/icons-svg/es/asn/ForwardOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![forward](data:image/svg+xml;base64,) */ 
const ForwardOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ForwardOutlinedSvg} />;

export default ForwardOutlined