// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FolderAddOutlinedSvg from '@ant-design/icons-svg/es/asn/FolderAddOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![folder-add](data:image/svg+xml;base64,) */ 
const FolderAddOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FolderAddOutlinedSvg} />;

export default FolderAddOutlined