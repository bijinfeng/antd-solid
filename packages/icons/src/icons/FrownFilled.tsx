// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FrownFilledSvg from '@ant-design/icons-svg/es/asn/FrownFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![frown](data:image/svg+xml;base64,) */ 
const FrownFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FrownFilledSvg} />;

export default FrownFilled