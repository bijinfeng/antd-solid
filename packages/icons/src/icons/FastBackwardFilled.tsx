// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FastBackwardFilledSvg from '@ant-design/icons-svg/es/asn/FastBackwardFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![fast-backward](data:image/svg+xml;base64,) */ 
const FastBackwardFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FastBackwardFilledSvg} />;

export default FastBackwardFilled