// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import CustomerServiceTwoToneSvg from '@ant-design/icons-svg/es/asn/CustomerServiceTwoTone.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![customer-service](data:image/svg+xml;base64,) */ 
const CustomerServiceTwoTone: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={CustomerServiceTwoToneSvg} />;

export default CustomerServiceTwoTone