// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import TableOutlinedSvg from '@ant-design/icons-svg/es/asn/TableOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![table](data:image/svg+xml;base64,) */ 
const TableOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={TableOutlinedSvg} />;

export default TableOutlined