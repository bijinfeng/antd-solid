// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import AreaChartOutlinedSvg from '@ant-design/icons-svg/es/asn/AreaChartOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![area-chart](data:image/svg+xml;base64,) */ 
const AreaChartOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={AreaChartOutlinedSvg} />;

export default AreaChartOutlined