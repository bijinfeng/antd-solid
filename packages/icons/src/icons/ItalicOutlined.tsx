// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ItalicOutlinedSvg from '@ant-design/icons-svg/es/asn/ItalicOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![italic](data:image/svg+xml;base64,) */ 
const ItalicOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ItalicOutlinedSvg} />;

export default ItalicOutlined