// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import InsertRowAboveOutlinedSvg from '@ant-design/icons-svg/es/asn/InsertRowAboveOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![insert-row-above](data:image/svg+xml;base64,) */ 
const InsertRowAboveOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={InsertRowAboveOutlinedSvg} />;

export default InsertRowAboveOutlined