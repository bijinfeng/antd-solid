// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import CustomerServiceFilledSvg from '@ant-design/icons-svg/es/asn/CustomerServiceFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![customer-service](data:image/svg+xml;base64,) */ 
const CustomerServiceFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={CustomerServiceFilledSvg} />;

export default CustomerServiceFilled