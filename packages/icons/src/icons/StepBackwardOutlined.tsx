// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import StepBackwardOutlinedSvg from '@ant-design/icons-svg/es/asn/StepBackwardOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![step-backward](data:image/svg+xml;base64,) */ 
const StepBackwardOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={StepBackwardOutlinedSvg} />;

export default StepBackwardOutlined