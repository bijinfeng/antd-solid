// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FunnelPlotTwoToneSvg from '@ant-design/icons-svg/es/asn/FunnelPlotTwoTone.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![funnel-plot](data:image/svg+xml;base64,) */ 
const FunnelPlotTwoTone: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FunnelPlotTwoToneSvg} />;

export default FunnelPlotTwoTone