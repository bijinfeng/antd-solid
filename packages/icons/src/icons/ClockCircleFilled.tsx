// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ClockCircleFilledSvg from '@ant-design/icons-svg/es/asn/ClockCircleFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![clock-circle](data:image/svg+xml;base64,) */ 
const ClockCircleFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ClockCircleFilledSvg} />;

export default ClockCircleFilled