// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import UsbOutlinedSvg from '@ant-design/icons-svg/es/asn/UsbOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![usb](data:image/svg+xml;base64,) */ 
const UsbOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={UsbOutlinedSvg} />;

export default UsbOutlined