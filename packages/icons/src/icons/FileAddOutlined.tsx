// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FileAddOutlinedSvg from '@ant-design/icons-svg/es/asn/FileAddOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![file-add](data:image/svg+xml;base64,) */ 
const FileAddOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FileAddOutlinedSvg} />;

export default FileAddOutlined