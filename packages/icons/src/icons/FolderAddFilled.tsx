// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FolderAddFilledSvg from '@ant-design/icons-svg/es/asn/FolderAddFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![folder-add](data:image/svg+xml;base64,) */ 
const FolderAddFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FolderAddFilledSvg} />;

export default FolderAddFilled