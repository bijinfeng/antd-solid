// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import LayoutFilledSvg from '@ant-design/icons-svg/es/asn/LayoutFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![layout](data:image/svg+xml;base64,) */ 
const LayoutFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={LayoutFilledSvg} />;

export default LayoutFilled