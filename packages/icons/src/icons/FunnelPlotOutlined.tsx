// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FunnelPlotOutlinedSvg from '@ant-design/icons-svg/es/asn/FunnelPlotOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![funnel-plot](data:image/svg+xml;base64,) */ 
const FunnelPlotOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FunnelPlotOutlinedSvg} />;

export default FunnelPlotOutlined