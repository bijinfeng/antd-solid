// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FileImageOutlinedSvg from '@ant-design/icons-svg/es/asn/FileImageOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![file-image](data:image/svg+xml;base64,) */ 
const FileImageOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FileImageOutlinedSvg} />;

export default FileImageOutlined