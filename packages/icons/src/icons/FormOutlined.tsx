// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FormOutlinedSvg from '@ant-design/icons-svg/es/asn/FormOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![form](data:image/svg+xml;base64,) */ 
const FormOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FormOutlinedSvg} />;

export default FormOutlined