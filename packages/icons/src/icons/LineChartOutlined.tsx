// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import LineChartOutlinedSvg from '@ant-design/icons-svg/es/asn/LineChartOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![line-chart](data:image/svg+xml;base64,) */ 
const LineChartOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={LineChartOutlinedSvg} />;

export default LineChartOutlined