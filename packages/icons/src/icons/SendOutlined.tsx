// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import SendOutlinedSvg from '@ant-design/icons-svg/es/asn/SendOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![send](data:image/svg+xml;base64,) */ 
const SendOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={SendOutlinedSvg} />;

export default SendOutlined