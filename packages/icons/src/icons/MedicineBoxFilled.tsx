// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import MedicineBoxFilledSvg from '@ant-design/icons-svg/es/asn/MedicineBoxFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![medicine-box](data:image/svg+xml;base64,) */ 
const MedicineBoxFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={MedicineBoxFilledSvg} />;

export default MedicineBoxFilled