// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import PlaySquareOutlinedSvg from '@ant-design/icons-svg/es/asn/PlaySquareOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![play-square](data:image/svg+xml;base64,) */ 
const PlaySquareOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={PlaySquareOutlinedSvg} />;

export default PlaySquareOutlined