// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import TrophyFilledSvg from '@ant-design/icons-svg/es/asn/TrophyFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![trophy](data:image/svg+xml;base64,) */ 
const TrophyFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={TrophyFilledSvg} />;

export default TrophyFilled