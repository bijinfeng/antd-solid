// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import PlayCircleOutlinedSvg from '@ant-design/icons-svg/es/asn/PlayCircleOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![play-circle](data:image/svg+xml;base64,) */ 
const PlayCircleOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={PlayCircleOutlinedSvg} />;

export default PlayCircleOutlined