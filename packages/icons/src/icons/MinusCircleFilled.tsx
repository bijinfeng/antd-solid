// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import MinusCircleFilledSvg from '@ant-design/icons-svg/es/asn/MinusCircleFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![minus-circle](data:image/svg+xml;base64,) */ 
const MinusCircleFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={MinusCircleFilledSvg} />;

export default MinusCircleFilled