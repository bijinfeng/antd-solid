// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import MediumCircleFilledSvg from '@ant-design/icons-svg/es/asn/MediumCircleFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![medium-circle](data:image/svg+xml;base64,) */ 
const MediumCircleFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={MediumCircleFilledSvg} />;

export default MediumCircleFilled