// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import PlusSquareFilledSvg from '@ant-design/icons-svg/es/asn/PlusSquareFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![plus-square](data:image/svg+xml;base64,) */ 
const PlusSquareFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={PlusSquareFilledSvg} />;

export default PlusSquareFilled