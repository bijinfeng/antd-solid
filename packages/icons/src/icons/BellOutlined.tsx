// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import BellOutlinedSvg from '@ant-design/icons-svg/es/asn/BellOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![bell](data:image/svg+xml;base64,) */ 
const BellOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={BellOutlinedSvg} />;

export default BellOutlined