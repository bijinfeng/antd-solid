// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import UnlockOutlinedSvg from '@ant-design/icons-svg/es/asn/UnlockOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![unlock](data:image/svg+xml;base64,) */ 
const UnlockOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={UnlockOutlinedSvg} />;

export default UnlockOutlined