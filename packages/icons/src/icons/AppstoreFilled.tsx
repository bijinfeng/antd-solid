// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import AppstoreFilledSvg from '@ant-design/icons-svg/es/asn/AppstoreFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![appstore](data:image/svg+xml;base64,) */ 
const AppstoreFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={AppstoreFilledSvg} />;

export default AppstoreFilled