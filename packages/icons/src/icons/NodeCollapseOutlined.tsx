// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import NodeCollapseOutlinedSvg from '@ant-design/icons-svg/es/asn/NodeCollapseOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![node-collapse](data:image/svg+xml;base64,) */ 
const NodeCollapseOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={NodeCollapseOutlinedSvg} />;

export default NodeCollapseOutlined