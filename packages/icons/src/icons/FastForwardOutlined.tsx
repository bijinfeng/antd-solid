// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FastForwardOutlinedSvg from '@ant-design/icons-svg/es/asn/FastForwardOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![fast-forward](data:image/svg+xml;base64,) */ 
const FastForwardOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FastForwardOutlinedSvg} />;

export default FastForwardOutlined