// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ArrowsAltOutlinedSvg from '@ant-design/icons-svg/es/asn/ArrowsAltOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![arrows-alt](data:image/svg+xml;base64,) */ 
const ArrowsAltOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ArrowsAltOutlinedSvg} />;

export default ArrowsAltOutlined