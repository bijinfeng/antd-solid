// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import MutedOutlinedSvg from '@ant-design/icons-svg/es/asn/MutedOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![muted](data:image/svg+xml;base64,) */ 
const MutedOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={MutedOutlinedSvg} />;

export default MutedOutlined