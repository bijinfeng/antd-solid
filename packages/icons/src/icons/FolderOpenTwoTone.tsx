// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FolderOpenTwoToneSvg from '@ant-design/icons-svg/es/asn/FolderOpenTwoTone.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![folder-open](data:image/svg+xml;base64,) */ 
const FolderOpenTwoTone: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FolderOpenTwoToneSvg} />;

export default FolderOpenTwoTone