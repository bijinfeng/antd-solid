// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FileAddFilledSvg from '@ant-design/icons-svg/es/asn/FileAddFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![file-add](data:image/svg+xml;base64,) */ 
const FileAddFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FileAddFilledSvg} />;

export default FileAddFilled