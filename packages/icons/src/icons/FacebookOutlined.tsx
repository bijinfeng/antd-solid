// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FacebookOutlinedSvg from '@ant-design/icons-svg/es/asn/FacebookOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![facebook](data:image/svg+xml;base64,) */ 
const FacebookOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FacebookOutlinedSvg} />;

export default FacebookOutlined