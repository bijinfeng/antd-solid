// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import CodeSandboxCircleFilledSvg from '@ant-design/icons-svg/es/asn/CodeSandboxCircleFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![code-sandbox-circle](data:image/svg+xml;base64,) */ 
const CodeSandboxCircleFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={CodeSandboxCircleFilledSvg} />;

export default CodeSandboxCircleFilled