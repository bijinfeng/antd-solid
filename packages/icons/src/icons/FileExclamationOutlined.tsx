// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import FileExclamationOutlinedSvg from '@ant-design/icons-svg/es/asn/FileExclamationOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![file-exclamation](data:image/svg+xml;base64,) */ 
const FileExclamationOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={FileExclamationOutlinedSvg} />;

export default FileExclamationOutlined