// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import StepForwardFilledSvg from '@ant-design/icons-svg/es/asn/StepForwardFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![step-forward](data:image/svg+xml;base64,) */ 
const StepForwardFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={StepForwardFilledSvg} />;

export default StepForwardFilled