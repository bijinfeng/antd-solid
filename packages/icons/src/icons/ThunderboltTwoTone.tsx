// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ThunderboltTwoToneSvg from '@ant-design/icons-svg/es/asn/ThunderboltTwoTone.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![thunderbolt](data:image/svg+xml;base64,) */ 
const ThunderboltTwoTone: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ThunderboltTwoToneSvg} />;

export default ThunderboltTwoTone