// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import HighlightFilledSvg from '@ant-design/icons-svg/es/asn/HighlightFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![highlight](data:image/svg+xml;base64,) */ 
const HighlightFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={HighlightFilledSvg} />;

export default HighlightFilled