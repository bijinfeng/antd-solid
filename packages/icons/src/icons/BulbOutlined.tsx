// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import BulbOutlinedSvg from '@ant-design/icons-svg/es/asn/BulbOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![bulb](data:image/svg+xml;base64,) */ 
const BulbOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={BulbOutlinedSvg} />;

export default BulbOutlined