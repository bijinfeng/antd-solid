// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ThunderboltFilledSvg from '@ant-design/icons-svg/es/asn/ThunderboltFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![thunderbolt](data:image/svg+xml;base64,) */ 
const ThunderboltFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ThunderboltFilledSvg} />;

export default ThunderboltFilled