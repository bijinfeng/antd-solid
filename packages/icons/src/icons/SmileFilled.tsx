// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import SmileFilledSvg from '@ant-design/icons-svg/es/asn/SmileFilled.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![smile](data:image/svg+xml;base64,) */ 
const SmileFilled: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={SmileFilledSvg} />;

export default SmileFilled