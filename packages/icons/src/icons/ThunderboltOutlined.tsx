// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import ThunderboltOutlinedSvg from '@ant-design/icons-svg/es/asn/ThunderboltOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![thunderbolt](data:image/svg+xml;base64,) */ 
const ThunderboltOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={ThunderboltOutlinedSvg} />;

export default ThunderboltOutlined