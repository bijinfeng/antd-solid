// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import PauseCircleOutlinedSvg from '@ant-design/icons-svg/es/asn/PauseCircleOutlined.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![pause-circle](data:image/svg+xml;base64,) */ 
const PauseCircleOutlined: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={PauseCircleOutlinedSvg} />;

export default PauseCircleOutlined