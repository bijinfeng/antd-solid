// GENERATE BY ./scripts/gen-icons.ts
// DON NOT EDIT IT MANUALLY

import type { AntdIconProps } from '../components/AntdIcon'
import BankTwoToneSvg from '@ant-design/icons-svg/es/asn/BankTwoTone.js'
import type { Component } from 'solid-js'
import AntdIcon from '../components/AntdIcon'

 /**![bank](data:image/svg+xml;base64,) */ 
const BankTwoTone: Component<AntdIconProps> = (props) => <AntdIcon {...props} icon={BankTwoToneSvg} />;

export default BankTwoTone